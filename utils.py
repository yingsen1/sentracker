...
def overlap_ratio(rect1, rect2):
    '''
    # 计算两个框的IOU重叠率
    :param rect1:  1.[x,y,w,h]-1dim/ 2.Nx[x,y,w,h] #（x,y）为左下角坐标
    :param rect2: the same as above
    :return: iou
    '''

    if rect1.ndim == 1: # 统一格式，使其为两维的格式
        rect1 = rect1[None, :]
    if rect2.ndim == 1:
        rect2 = rect2[None, :]
...
def crop_image(img, bbox, img_size=107, padding=16, valid=False):
    ...
    :param img_size: outout_size
    :param padding: ??
    :param valid: confirm size ration
    :return: output_img_in_bbox(accordingToImageSize)
    '''
    x, y, w, h = np.array(bbox, dtype='float32') # dtype as FloatTensor
    ...
    img_h, img_w, _ = img.shape # _ is channel
